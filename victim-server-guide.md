...
🎯 Victim Server - Complete Overview
📋 What is the Victim Server?

The Victim Server is a deliberately breakable web application built with FastAPI and Redis. It is designed to simulate real-world server failures for testing and training fault diagnosis systems (such as a Probabilistic Graphical Model).

Think of it as a "patient" for your ML model — you can make it "sick" using different faults, and your model must diagnose the issue based on observed symptoms.
...
│  │                   FastAPI App                       │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌─────────┐   │    │
│  │  │ /api/products │  │  /api/users  │  │ /health │   │    │
│  │  └──────┬───────┘  └──────┬───────┘  └────┬────┘   │    │
│  │         │                  │                │        │    │
│  │  ┌──────▼──────────────────▼────────────────▼────┐  │    │
│  │  │           Fault Injection System              │  │    │
│  │  │  ┌──────────┐ ┌────────┐ ┌────────┐ ┌──────┐ │  │    │
│  │  │  │CPU Spike │ │Memory  │ │Latency │ │Error │ │  │    │
│  │  │  │   🔥     │ │Leak 🧠 │ │  🐌    │ │Rate💥│ │  │    │
│  │  │  └──────────┘ └────────┘ └────────┘ └──────┘ │  │    │
│  │  └──────────────────────────────────────────────┘  │    │
│  └────────────────────────────────────────────────────┘    │
...
Handles business logic for products and users

Built with Python 3.12+ using async/await

2. Redis Database

Stores product catalog and user data

Provides persistence between requests

Accessible at localhost:6379

3. Fault Injection System

Four distinct fault types can be triggered via API:

Fault	Trigger Endpoint	What It Does
CPU Spike 🔥	/fault/cpu/start	Consumes CPU cycles, slows all requests
Memory Leak 🧠	/fault/memory/start	Gradually consumes RAM, causes slowdown
API Latency 🐌	/fault/latency/start	Adds 1–3 second delay to responses
Error Rate 💥	/fault/errors/start	~30% of requests return 500 errors
4. Monitoring & Metrics
Endpoint	Purpose
/health	Real-time system metrics
/api/metrics	Structured output for PGM
/api/debug	Internal state inspection
